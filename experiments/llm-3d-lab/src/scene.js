...
/* الكانفس في مستند RTL يرث الاتجاه؛ والنص المختلط (عربي + softmax) او الاعرض من الكانفس
   الثابت كان يقص او يعاد ترتيبه فيبدو مبتورا. الحل: نقيس النص ونوسع الكانفس ليتسع له تماما
   (لا قص ابدا)، ونحدد الحجم بارتفاع عالمي ثابت فالعرض يتبع نسبة النص، مع اتجاه صريح. */
function labelSprite(text, color, worldH = 22, font = 40, dir = "rtl") {
  const m = (labelSprite._ctx || (labelSprite._ctx = document.createElement("canvas").getContext("2d")));
  m.font = `700 ${font}px Segoe UI, Tahoma, sans-serif`;
  const pad = Math.round(font * 0.7);
  const w = Math.ceil(m.measureText(text).width) + pad * 2;
  const h = Math.round(font * 1.9);
  const c = document.createElement("canvas");
...
function buildStatic() {
  ...
  mk("الرمز التالي: توزيع الاحتمالات الحي", 205, "#ffd166");
}
