...
const DATA_DIR = join(ROOT, "data", "academic");
const AUDIT_FILE = join(DATA_DIR, "academic-source-verification.json");
const REPORT_FILE = join(ROOT, "docs", "academic", "ACADEMIC_SOURCE_AUDIT.md");
const WEB_FILES = ["academic-source-web-verification.json", "academic-source-web-verification-extra.json"].map((name) => join(DATA_DIR, name));

function currentCatalog() {
  const source = fs.readFileSync(join(ROOT, "src", "templates", "atlas-template.html"), "utf8");
...
const manual = JSON.parse(fs.readFileSync(join(DATA_DIR, "academic-source-manual-evidence.json"), "utf8"));
