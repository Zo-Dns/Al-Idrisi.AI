...
import { JOURNEY, JOURNEY_SOURCES, NODES } from "../src/content/ai-content.mjs";
...
const template = readFileSync(new URL("../src/templates/atlas-template.html", import.meta.url), "utf8");
...
for (const file of ["atlas-template.html", "ai-network-template.html"]) {
  const content = readFileSync(new URL(`../src/templates/${file}`, import.meta.url), "utf8");
...
for (const file of ["ai-how-ai-works.html"]) {
  const content = readFileSync(new URL(`../pages/${file}`, import.meta.url), "utf8");
  assert.ok(content.includes(REVIEWED_BUTTON), `${file}: عنوان الجولة الكبرى المصحح مفقود`);
  assert.ok(!content.includes("عوالم الذكاء الاصطناعي من التعلم الى المسؤولية"), `${file}: العنوان القديم عاد`);
