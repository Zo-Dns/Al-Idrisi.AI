...
const build = readFileSync("scripts/atlas-build.mjs", "utf8");
const template = readFileSync("src/templates/atlas-template.html", "utf8");
...
const contentFiles = ["ai", "llm", "dl", "ml", "data", "ethics", "apps", "classic", "rl", "prob", "history"]
  .map((name) => readFileSync(`src/content/${name}-content.mjs`, "utf8"))
  .join("\n");
...
const legacyContextLocations = [
  ...
  ["قالب الشبكة الأقدم", readFileSync("src/templates/ai-network-template.html", "utf8")],
...
check(finalAtlas.startsWith('<!doctype html>\n<html lang="ar" dir="rtl">') && finalAtlas.includes('<head>\n<meta charset="utf-8">') && finalAtlas.includes('<meta name="description"') && finalAtlas.includes('<title>الإدريسي — Al-Idrisi.AI — كيف يعمل الذكاء الاصطناعي</title>\n</head>\n<body>\n<style>'), "ملف الأطلس النهائي وثيقة HTML معيارية كاملة بترميز UTF-8 ووصف وعنوان داخل head");
check(!existsSync("pages/ai-how-ai-works-standalone.html"), "لا توجد نسخة أطلس مكررة باسم standalone");
